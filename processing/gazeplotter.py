...
def draw_scanpath(fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None):
    ...
    fix = parse_fixations(fixations)
    # draw fixations
    ax.scatter(fix['x'], fix['y'], s=fix['dur'] * 3, c=COLS['chameleon'][2], marker='o', cmap='jet', alpha=alpha,
               edgecolors='none')
    ...
    if saccades:
        ...
        for st, et, dur, sx, sy, ex, ey in saccades:
            ...
            ax.arrow(sx, sy, ex - sx, ey - sy, alpha=alpha, fc=COLS['aluminium'][2], ec=COLS['aluminium'][5], fill=True,
                     shape='full', width=15, head_width=20, head_starts_at_zero=False, overhang=0)
